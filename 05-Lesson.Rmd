...
#### Fit an aspatial regression model to predict house values
...
Interest in spatial statistical models has increased substantially in the recent years stemming in part from the availability of methods to fit them. One such method is called Bayesian inference.
...
Latent Gaussian fields appear in a fair number of spatial models. This means that INLA is useful for fitting more complex spatial regression models when only marginal inference on the model parameters is needed.
