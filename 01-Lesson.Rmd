...
Your final grade will be based equally on your grades on 4-6 assignments given throughout the semester and on your end-of-the-semester class project.

We will use RStudio Cloud throughout the semester. If you prefer to work with R and RStudio on your own computer (without RStudio Cloud) then
...
You will need to Export the lesson Rmd file and make sure all the required packages are loaded.
...
Table the student's drink preference.
```{r}
table(df$Drink)
```
...
Finally we load the shapefile into R. We use the `read_sf()` function from the **sf** package. We specify the data source name `dsn =`
```{r}
library(sf)
...
Subset the data for years (`yr`) since 2001 and for EF ratings (`mag`) greater than 0.
```{r}
