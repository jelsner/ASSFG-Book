...
Here we specify the model to predict the prevalence of malaria in The Gambia using the SPDE approach and functions from the {INLA} package.

Conditional on the true prevalence $P({\bf x}_i)$ at location ${\bf x}_i$, i = 1, $\ldots, n$, the number of positive results $Y_i$ out of $N_i$ people sampled follows a binomial distribution
$$
Y_i|P({\bf x}_i) \sim \hbox{Binomial}(N_i, P({\bf x}_i)) \\
\hbox{logit}(P({\bf x}_i)) = \beta_0 + \beta_1 \times \hbox{altitude} + S({\bf x}_i))
$$
