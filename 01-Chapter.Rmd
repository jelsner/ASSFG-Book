...
First, install the package with the `install.packages()` function. Then to make the functions available use the `library()` function as follows.
```{r}
...
Then we load the shapefile into R. We use the `read_sf()` function from the {sf} package. We specify the data source name `dsn =`. The name of the file exists outside of R in your project directory so it needs to be inside quotes (either single or double).
```{r}
install.packages("sf")
library(sf)
