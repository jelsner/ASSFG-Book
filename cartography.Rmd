...
From: https://github.com/riatelab/cartography/blob/master/vignettes/cartography.Rmd

The aim of the {cartography} package is to obtain thematic maps with the visual quality of those build with a classical mapping or GIS software. It uses {sf} simple feature data frames to produce {base} graphics.

##  Features

Functions in {cartography} are classified in the following categories :
...
Each function has two main arguments that are: `x`, a spatial object (preferably an `sf` object), `var`, the name of a variable to map.  

Objects of class {sp} are handled through the `spdf` argument if the variable is contained within the `Spatial*DataFrame` and through `spdf`, `spdfid`, `df`, `dfid` if the variable is in a separate `data.frame` that needs to be joined to the `Spatial*DataFrame`. 
...
A set of functions is dedicated to the creation or _transformation_ of spatial objects (e.g. borders extraction, grid or links creation). These functions are provided to ease the creation of some more advanced maps that usually need geo-processing. 

Along with the cartographic functions, some other functions are dedicated to _layout design_ (e.g. customizable scale bar, north arrow, title, sources or author information...).

The 16 original color palettes are shipped within the package. Those palettes can be customized and combined. 

Legends are displayed by default along cartographic layers but more parameters are available through `legend*()` functions.

The function `getBreaks()` give access to most of the classification methods used for data binning. 

## OpenStreetMap Basemap and Proportional Symbols

As an example of a thematic map.
